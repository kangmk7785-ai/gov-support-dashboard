...
def fetch_subsidy24():
    ...
    for page in range(1, 4):
        try:
            ...
            if page == 1:
                print(f"  🔍 XML 태그: {sorted(all_tags)[:25]}")

            # 다양한 태그명으로 시도
            name_tags = ['servNm', 'wlfareInfoNm', 'SERV_NM', 'servNM']
            org_tags = ['jurMnofNm', 'ministryNm', 'JUR_MNOF_NM', 'deptNm']
            desc_tags = ['servDgst', 'wlfareInfoOutline', 'SERV_DGST', 'servDtlLink']
            target_tags = ['trgterIndvdlArray', 'trgterIndvdlNm', 'TRGTER_INDVDL_NM', 'lifeNm']
            id_tags = ['servId', 'SERV_ID', 'wlfareInfoId']
            link_tags = ['servDtlLink']

            def find_text(el, tag_list):
                for t in tag_list:
                    val = el.findtext(t)
            ...
            # 반복 가능한 요소 찾기
            containers = root.findall('.//servList')
            if not containers:
                containers = root.findall('.//item')
            if not containers:
                containers = root.findall('.//row')
            if not containers:
                # body 아래 모든 직계 자식 시도
                body = root.find('.//body')
                if body is not None:
            ...
            for item in containers:
                title = find_text(item, name_tags)
                if not title:
                    continue
                org = find_text(item, org_tags)
                desc = find_text(item, desc_tags)
                target = find_text(item, target_tags)
                svc_id = find_text(item, id_tags)
                svc_link = find_text(item, link_tags)
                detail_url = svc_link if svc_link else (f"https://www.bokjiro.go.kr/ssis-tbu/twataa/wlfareInfo/moveTWAT52011M.do?wlfareInfoId={svc_id}" if svc_id else "https://www.bokjiro.go.kr")
