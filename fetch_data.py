...
def generate_html(programs):
    ...
    # data.json에서 읽어오는 방식의 HTML 생성
    programs_json = json.dumps(programs, ensure_ascii=False, indent=2)
    ...
    if html_template_path.exists():
        ...
        html = template.replace("/*__PROGRAMS_DATA__*/", f"const P = {programs_json};")
        html = html.replace("__UPDATE_TIME__", update_time)
        html = html.replace("__TOTAL_COUNT__", str(len(programs)))
    else:
        # 템플릿이 없으면 인라인으로 생성
        html = generate_inline_html(programs, update_time)
